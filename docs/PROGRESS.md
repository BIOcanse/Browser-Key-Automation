...
- Chrome Web Store 首次上传交付已落地：扩展新增确定性 SVG 品牌主稿及 16/32/48/128 RGBA PNG，清单同时声明安装图标和 action 图标；独立打包器从扩展运行产物建立根层 `manifest.json` 的 keyless `0.0.0.1` 首传 ZIP，并把 SHA-256 与后续身份同步说明放在 ZIP 外。该包只用于 Dashboard 建立条目，不在取得 public key/Item ID、同步 Zig App exact-Origin 门禁并递增版本前发布。Web Store 包 68 个文件的根层、图标、locale、禁入文件、解压一致性与摘要已自动验证；完整 Node/Zig 单元回归、开发分包 69/14/14 文件校验以及隔离 Chromium 管理页/扩展—relay 两套 smoke 通过。详见[交付切片](implementation/chrome-web-store-delivery.md)。
