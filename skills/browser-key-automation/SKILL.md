...
- Tree rows with a NodeRef can be passed to `dom.describe`, `dom.click`, `dom.click.real`, `dom.focus`, `dom.insertText`, `dom.scroll`, `dom.setValue`, `dom.select`, or a targeted native-keyboard command. Re-read the descriptor when the page may have changed.
...
- Use `dom.insertText` for immediate untrusted plain-text insertion at the current caret or selection. Use `keyboard.type` for immediate trusted Unicode input, `keyboard.typeHuman` only when explicit human cadence and bounded corrections are needed, and `keyboard.press` for named keys, chords, shortcuts, or deliberate raw `down`/`up` state. A plain key or chord always performs a complete down/up. `targetRef` may be a NodeRef or TabRef; native keyboard input activates and verifies the exact foreground Chromium window. `keyboard.reset` releases only keys held for the current extension instance and should be called before abandoning a flow that used raw `down`. Treat `partially_sent` and `unknown` as non-retryable; inspect `heldKeys` and reset explicitly when cleanup is needed.
