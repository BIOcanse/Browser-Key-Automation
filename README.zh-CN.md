...
Agent可以继续使用已经登录的 Chromium 标签页，按需阅读网页、执行操作，无缝接管。

## 为什么用它

- **页面很大，视图可以很小。** 缓存的操作树保留整体结构，只展开需要的分支。切换标签页后，只要文档未变，展开状态仍在；层级、区间、子树是一次性视图选择，而不是有损改写网页。
...
> 每次全新安装都会生成相同的**公开 Root 试用 Key**，具体值在 Agent skill 中。它不是私人凭据。个人浏览器请创建私人 Key、切换客户端，再撤销试用 Key；仅创建新 Key 不会使旧 Key 失效。升级不会恢复。
...
BKA 是目前面向个人agent助手这一场景最全面的浏览器自动化插件，一次授权，随时使用。真正让agent成为你的助手。
