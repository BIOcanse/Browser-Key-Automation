...
沿用已登入的 Chromium 分頁，按需閱讀網頁、執行操作，再儲存結果。日常指令使用擴充功能權限：不用另開自動化瀏覽器、不自動附加偵錯器，也不增加逐條確認。瀏覽器自身的權限提示仍然保留。

## 為什麼使用

- **網頁很大，檢視可以很小。** 快取操作樹保留整體結構，只展開需要的分支。切換分頁後，只要文件未變，展開狀態仍在；深度、區間與子樹是一次性檢視選擇，不是有損改寫。
...
> 每次全新安裝都會建立相同的**公開 Root 試用 Key**，具體值在 Agent skill 中。它不是私人憑證。個人瀏覽器請建立私人 Key、切換用戶端，再撤銷試用 Key；只建立新 Key 不會停用舊 Key。更新不會補發或恢復它。
...
BKA 專注於低摩擦接入個人瀏覽器、按需展開的操作樹，以及連貫的操作與檔案流程，不宣稱全面取代測試框架或 DevTools。
