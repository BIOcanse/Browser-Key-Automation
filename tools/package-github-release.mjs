...
function releaseNotes(releaseVersion, assets) {
  const lines = [
    ...
    "",
    "This release adds verified state-change workflows and native keyboard input to the existing Key-scoped Chromium automation surface.",
    "",
    "## Highlights",
    "",
    "- Use `ensure.run` to combine a strict condition, one registered action, bounded preparation, and an observable goal under one deadline. Nested-scroll and virtual-list search are supported, live iframe paths are re-resolved during observation, and admitted calls return Key-owned bounded, redacted traces.",
    "- Send exact Unicode text, human-paced text, named keys, arbitrary shortcuts, or deliberate key down/up sequences through the Windows native keyboard backend. `keyboard.reset` releases only keys held by the current extension instance.",
    "- Insert text at the current DOM caret with `dom.insertText`. Native keyboard operations remain independent permissions and require the exact Chromium window in the foreground.",
    "- The public surface now contains 59 commands, 48 permissions, 128 schemas, and 105 explicit Freedom Points. Existing page trees, files, demonstrations, transparent element screenshots, native click, and explicit CDP debugging remain available.",
    "",
    "## Downloads",
    "",
