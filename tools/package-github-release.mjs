...
function releaseNotes(releaseVersion, assets) {
  const lines = [
    ...
    "",
    "This release adds transparent element screenshots and explicit CDP debugging to the existing Key-scoped Chromium automation surface.",
    "",
    "## Highlights",
    "",
    "- Capture a Canvas, chart, SVG shape, or container from an existing NodeRef as an exact-size transparent PNG. Visible descendants, supported clipping geometry, aspect ratio, and relative layout are preserved.",
    "- Use `debugger.attach`, `debugger.send`, `debugger.events.get`, and `debugger.detach` through a separate Key permission when a task needs CDP. Routine extension operations never attach automatically, and Chrome retains its own debugging UI.",
    "- Save an element image in one command with `element-shot`; large CDP results can be requested as bounded Artifacts.",
    "",
    "## Downloads",
    "",
