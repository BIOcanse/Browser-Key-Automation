...
try {
  ...
  for (const method of ["input.calibrate", "virtualMouse.move", "virtualKeyboard.input"]) {
    const response = await runCli(["call", "--method", method, "--read-timeout-ms", "5000"], apiKey);
    assert.equal(response.json.error.code, "EXTENSION_UNAVAILABLE", `${method} must pass CLI method validation`);
  }
