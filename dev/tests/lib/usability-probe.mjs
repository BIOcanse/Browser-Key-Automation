...
export async function runUsabilityProbe({ forward, cliPath, apiKey, instanceRef, sampleRoot, baseUrl, windowId, gates, workerClient }) {
  ...
  try {
    ...
    let saved;
    try {
      saved = await runCli(cliPath, ["page-save", "--tab-ref", tabRef, "--output", output,
        "--instance", instanceRef, "--api-key-env", "BKA_SMOKE_SAVE_KEY"], apiKey);
    } catch (error) {
      // This is a NEW capture, not the original failing Blob. The original
      // operation's correlated stage diagnostics establish its failure phase.
      if (workerClient) {
        const diagnostic = await pageEvaluate(workerClient, async ({ url }) => {
          const matches = (await chrome.tabs.query({})).filter((tab) => tab.url === url);
          const result = { matchingTabs: matches.length, blob: null, readBytes: null, error: null };
          if (matches.length !== 1) return result;
          try {
            const blob = await chrome.pageCapture.saveAsMHTML({ tabId: matches[0].id });
            result.blob = { size: blob?.size, type: blob?.type };
            result.readBytes = (await blob.arrayBuffer()).byteLength;
          } catch (failure) { result.error = { name: failure?.name, message: failure?.message }; }
          return result;
        }, { url: `${baseUrl}usability` }).catch((failure) => ({ probeError: String(failure) }));
        await writeFile(path.join(sampleRoot, "page-save-failure.json"), JSON.stringify(diagnostic, null, 2) + "\n");
      }
      throw error;
    }
