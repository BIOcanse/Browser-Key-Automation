...
import { monitorNativeCrash } from "./lib/native-crash-monitor.mjs";
...
import { runPageSaveInvestigation } from "./lib/page-save-investigation.mjs";
...
const commandRegistry = JSON.parse(await readFile(path.join(workspaceRoot, "dev", "registries", "commands.registry.json"), "utf8"));
const freedomRegistry = JSON.parse(await readFile(path.join(workspaceRoot, "dev", "registries", "freedom.registry.json"), "utf8"));
...
const chromium = spawn(
  ...
  [
    ...
    ...(virtualMouseOnly ? ["--enable-logging=stderr"] : []),
...
let chromiumStderr = "";
const browserProcessEvents = [];
chromium.on("exit", (code, signal) => {
  browserProcessEvents.push({ kind: "browser-exit", code, signal, at: Date.now() });
  console.error(JSON.stringify(browserProcessEvents.at(-1)));
});
...
try {
  if (virtualMouseOnly && process.env.BKA_TEST_PROCDUMP) {
    crashMonitor = await monitorNativeCrash({ executable: process.env.BKA_TEST_PROCDUMP, browserPid: chromium.pid,
      directory: path.join(workspaceRoot, "out", "test-artifacts", "native-crash") });
  }
  ...
  const domPointer = debuggerElementOnly || virtualMouseOnly || pageSaveInvestigation || recordingOnly || browserFeaturesOnly || adminActionsOnly ? null : await runDomPointerProbe({
    ...
    scopedForward: async (permissions) => {
      const scoped = await pageEvaluate(pageClient, async ({ permissions, mutationId }) => {
        const service = await import(chrome.runtime.getURL("background/key-service.js"));
        return service.createKey({ mutationId, displayName: "Pointer scoped probe", keyKind: "regular", permissions, enabled: true, expiresAt: null });
      }, { permissions, mutationId: adminMutationId() });
      return (method, params) => forwardCommand(nativeClient, targetInstance, scoped.apiKey, `pointer-scoped-${++pointerSequence}`, method, params);
    },
    sampleRoot: path.join(workspaceRoot, "out", "test-artifacts", "dom-pointer"), baseUrl: testPageUrl, windowId: coreTab.windowId, workerClient,
  });

  let usabilitySequence = 0;
  const usability = focusedProbeOnly ? null : await runUsabilityProbe({
    ...
    cliPath: path.join(workspaceRoot, "app", "client", "src", "main.mjs"), apiKey: createdKey.apiKey,
    ...
    windowId: coreTab.windowId, gates: usabilityGates, workerClient,
  });

  let shotDemoSequence = 0;
  const shotDemo = focusedProbeOnly ? null : await runShotDemoProbe({
  ...
  if (focusedProbeOnly) {
    ...
    if (pageSaveInvestigation) {
      const result = await runPageSaveInvestigation({
        ...
        forward: (method, params) => forwardCommand(nativeClient, targetInstance, createdKey.apiKey, `save-investigation-${++usabilitySequence}`, method, params),
        cliPath: path.join(workspaceRoot, "app", "client", "src", "main.mjs"), apiKey: createdKey.apiKey,
        instanceRef: `${targetInstance.relayEpoch}/${targetInstance.instanceNumber}`,
        sampleRoot: path.join(workspaceRoot, "out", "test-artifacts", "page-save-investigation"),
        baseUrl: testPageUrl, windowId: coreTab.windowId, gates: usabilityGates, workerClient,
      });
      console.log(JSON.stringify({ pageSaveInvestigation: result }));
      assert.equal(result.failures, 0, "MHTML investigation recorded failed saves; inspect its original-operation evidence");
      assert.equal(result.incompleteDiagnostics, 0, "MHTML investigation lost operation diagnostics; do not infer a failure stage");
    }
    const runMouseProbe = nativeBoundaryMode ? (await import("./lib/native-boundary-probe.mjs")).runNativeBoundaryProbe : runVirtualMouseProbe;
    const virtualMouse = virtualMouseOnly ? await runMouseProbe({
      forward: (method, params) => forwardCommand(nativeClient, targetInstance, createdKey.apiKey, `mouse-${++pointerSequence}`, method, params),
      sampleRoot: path.join(workspaceRoot, "out", "test-artifacts", "virtual-mouse-browser"), baseUrl: testPageUrl,
      windowId: coreTab.windowId, workerClient, realInputAcceptance, mode: nativeBoundaryMode,
      browserVersion: nativeBoundaryMode ? await browserClient.send("Browser.getVersion") : undefined,
      scopedForward: async () => {
        const scoped = await pageEvaluate(pageClient, async ({ permissions, mutationId }) => {
          const service = await import(chrome.runtime.getURL("background/key-service.js"));
          return service.createKey({ mutationId, displayName: "Input handoff probe", keyKind: "regular", permissions, enabled: true, expiresAt: null });
        }, { permissions: expectedActivePermissionIds, mutationId: adminMutationId() });
        return {
          forward: (method, params) => forwardCommand(nativeClient, targetInstance, scoped.apiKey, `input-scoped-${++pointerSequence}`, method, params),
          revoke: () => pageEvaluate(pageClient, ({ requestId, params }) => new Promise((resolve, reject) => {
            const port = chrome.runtime.connect({ name: "browser-key-automation.admin.v1" });
            const timer = setTimeout(() => { port.disconnect(); reject(new Error("Admin revoke timed out")); }, 10_000);
            port.onDisconnect.addListener(() => { clearTimeout(timer); reject(new Error("Admin revoke disconnected")); });
            port.onMessage.addListener((response) => {
              if (response.requestId !== requestId) return;
              clearTimeout(timer);
              if (response.ok) resolve(response.result); else reject(new Error(JSON.stringify(response.error)));
              port.disconnect();
            });
            port.postMessage({ requestId, method: "keys.revoke", params });
          }), { requestId: `ui1.${randomBytes(16).toString("base64url")}`, params: {
            mutationId: adminMutationId(), keyId: scoped.key.keyId, expectedRevision: scoped.key.recordRevision,
          } }),
        };
      },
      recycleWorker: async () => {
        const target = await waitForTarget(debugPort, (item) => item.type === "service_worker" && item.url === `chrome-extension://${extensionId}/background.js`);
        assert.equal((await browserClient.send("Target.closeTarget", { targetId: target.id })).success, true);
        workerClient.close();
        await pageEvaluate(pageClient, () => chrome.runtime.sendMessage({ kind: "bka-test-wake" }));
        workerClient = await waitForExtensionWorker(debugPort);
        return workerClient;
      },
    }) : null;
    console.log(JSON.stringify({ ok: true, debuggerElement, domPointer, virtualMouse }));
  } else {
  ...
  if (relayOutput) console.error(relayOutput.slice(-8000));
  console.error(JSON.stringify({ browserProcessEvents, browserExitCode: chromium.exitCode, browserSignal: chromium.signalCode }));
  if (chromiumStderr) console.error(chromiumStderr.slice(-8000));
  throw error;
} finally {
  await crashMonitor?.stop();
  await writeFile(path.join(workspaceRoot, "out", "test-artifacts", "relay-native.log"), relayOutput);
  if (nativeBoundaryMode) await writeFile(path.join(workspaceRoot, "out", "test-artifacts", "native-browser.log"), chromiumStderr);
  nativeClient?.close();
  interruptionClient?.close();
  pageClient?.close();
  browserClient?.close();
...
async function forwardCommand(
  ...
  while (Date.now() < deadline) {
    const response = await client.readJson(Math.max(1, deadline - Date.now()));
    if (response.kind === "transport.error" || response.payload?.clientRequestId === clientRequestId || response.clientRequestId === clientRequestId || response.payload?.clientRequestId === "") return response;
    // A previously timed-out command may complete during cleanup. Never use
    // that unrelated response as evidence that the cleanup itself succeeded.
    console.warn(JSON.stringify({ lateResponse: response.kind, requestId: response.payload?.clientRequestId ?? response.clientRequestId, awaiting: clientRequestId }));
  }
  throw new Error(`Timed out awaiting ${clientRequestId}`);
}
