...
try {
  ...
  assert.equal(allocated.kind, "native.virtualInput.result");
  const inputId = allocated.ok ? String(allocated.result.input.id) : "1";
  if (process.platform === "win32") {
    assert.equal(allocated.ok, true);
    assert.equal(allocated.result.input.keyboard.keys.length, 256);
    assert.ok(allocated.result.input.keyboard.keys.every((key) => key === 0));
  }
  firstExtension.sendJson({
    kind: "native.input.keyboard",
    requestId: "nk1.reset-route-owned",
    inputId, windowId: null, viewport: { width: 1, height: 1 },
