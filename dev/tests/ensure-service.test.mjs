...
function harness() {
  ...
  const dependencies = {
    ...
    executeAction: async (currentAction, resolvedNodeRef, timeoutMs) => {
      state.actions += 1;
      state.onAction?.(currentAction, resolvedNodeRef, timeoutMs);
      if (state.hangAction) return new Promise(() => {});
      return { applied: true };
    },
    prepareAction: async (...args) => { await state.onPrepare?.(...args); },
...
test("ensure preparation precedes effect, carries remaining budget, and never runs in strict or already satisfied", async () => {
  const h = harness(), order = [];
  h.state.onPrepare = (_action, _node, timeoutMs) => { order.push("prepare"); assert.equal(timeoutMs, 300); h.state.now += 40; };
  h.state.onAction = (_action, _node, timeoutMs) => { order.push("effect"); assert.equal(timeoutMs, 260); };
  const result = await h.run(request());
  assert.equal(result.status, "satisfied"); assert.deepEqual(order, ["prepare", "effect"]);
  const strict = harness(); strict.state.onPrepare = () => assert.fail("strict must not auto-calibrate");
  strict.state.active = true;
  await strict.run(request({ mode: "strict", precondition: { kind: "tab_active", tabRef } })); assert.equal(strict.state.actions, 1);
  const done = harness(); done.state.active = true; done.state.onPrepare = () => assert.fail("goal already satisfied");
  await done.run(request({ goal: { kind: "tab_active", tabRef } })); assert.equal(done.state.actions, 0);
});

test("failed, hanging, and expired preparation sends no effect", async () => {
  const failed = harness(); failed.state.onPrepare = () => { throw Object.assign(new Error("CalibrationRequired"), { code: "NATIVE_INPUT_FAILED" }); };
  const result = await failed.run(request());
  assert.equal(result.status, "failed"); assert.equal(result.stage, "prepare"); assert.equal(result.effectSent, false); assert.equal(failed.state.actions, 0);
  assert.ok(!failed.state.traceEvents.some((event) => event.operation === "effect_entered"));
  const hanging = harness(); hanging.state.onPrepare = () => new Promise(() => {});
  const pending = hanging.run(request()); await hanging.advance(301);
  assert.equal((await pending).effectSent, false); assert.equal(hanging.state.actions, 0);
  const expired = harness(); expired.state.onPrepare = () => { expired.state.now = 301; };
  await expired.run(request()); assert.equal(expired.state.actions, 0);
});
