...
assert.equal(manifest.version, JSON.parse(await readFile(path.join(workspaceRoot, "extension", "manifest.json"), "utf8")).version);
...
assert.equal((await readFile(path.join(windowsAppRoot, "virtual-mouse-hook.dll"))).subarray(0, 2).toString("ascii"), "MZ");
assert.match(await readFile(path.join(windowsAppRoot, "licenses", "MinHook.txt"), "utf8"), /Tsuda Kageyu/u);
...
for (const appRoot of [windowsAppRoot, linuxAppRoot]) {
  ...
  assert.equal(
    ...
    await readFile(path.join(workspaceRoot, "dev", "registries", "commands.registry.json"), "utf8"),
  );
  assert.equal(
    await readFile(
...
async function resolveCurrentWindowsAppRoot(root, baseName) {
  const expectedCommands = await readFile(path.join(workspaceRoot, "dev", "registries", "commands.registry.json"), "utf8");
  const expectedFreedom = await readFile(path.join(workspaceRoot, "dev", "registries", "freedom.registry.json"), "utf8");
