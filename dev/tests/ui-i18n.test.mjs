...
test("UI catalog has 20 complete choices including separate Simplified and Traditional Chinese", () => {
  ...
  for (const { tag, nativeName } of UI_LOCALES) {
    ...
    assert.ok(UI_MESSAGES[tag].trialKeyTitle.trim(), `${tag}: public trial label`);
    assert.ok(UI_MESSAGES[tag].trialKeyWarning.trim(), `${tag}: public trial notice`);
    if (tag !== "en") assert.notEqual(UI_MESSAGES[tag].trialKeyWarning, UI_MESSAGES.en.trialKeyWarning);
  }
});
...
test("permission groups are an explicit complete projection of active atoms with all 20 labels", async () => {
  const ui = JSON.parse(await readFile(path.join(workspaceRoot, "dev/registries/ui.registry.json"), "utf8"));
  const commands = JSON.parse(await readFile(path.join(workspaceRoot, "dev/registries/commands.registry.json"), "utf8"));
