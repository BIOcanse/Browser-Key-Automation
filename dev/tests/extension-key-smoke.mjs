...
async function main() {
  ...
  try {
    ...
    const setupSnapshot = () => ({
      ...
      trialWarning: document.querySelector("[data-trial-key-warning]")?.textContent,
      trialWarningVisible: document.querySelector("[data-trial-key-warning]")?.getBoundingClientRect().height > 0,
    ...
    assert.match(welcomeBefore.trialWarning, /public and shared.*revoke/su);
    assert.equal(welcomeBefore.trialWarningVisible, true);
    ...
    const trialSamples = [];
    const installedTrial = await pageEvaluate(pageClient, trialSnapshot);
    assert.equal(installedTrial.key.keyKind, "root");
    assert.equal(installedTrial.auth, "accepted");
    assert.equal(installedTrial.fixedSecretMatches, true);
    assert.equal(installedTrial.warningVisible, true);
    trialSamples.push({ step: "fresh_install", ...installedTrial });
    const trialKeyId = installedTrial.key.keyId;
    const localeWarnings = await pageEvaluate(pageClient, async () => {
      ...
      for (const { tag } of UI_LOCALES) {
        ...
        samples.push({ lang: document.documentElement.lang, text, correct: text === UI_MESSAGES[tag].trialKeyWarning,
          visible: !notice.hidden && notice.getBoundingClientRect().height > 0, fits: notice.scrollWidth <= notice.clientWidth });
      }
      select.value = previous; select.dispatchEvent(new Event("change", { bubbles: true }));
      return samples;
    });
    assert.equal(localeWarnings.length, 20);
    assert.ok(localeWarnings.every((sample) => sample.correct && sample.visible && sample.fits));
    trialSamples.push({ step: "all_20_locales", samples: localeWarnings });
    const trialImage = await pageClient.send("Page.captureScreenshot", { format: "png", fromSurface: true });
    await writeFile(path.join(workspaceRoot, "out/test-artifacts/public-trial-key.png"), Buffer.from(trialImage.data, "base64"));
    await writeFile(path.join(workspaceRoot, "out/test-artifacts/public-trial-key.html"),
      await pageEvaluate(pageClient, () => document.documentElement.outerHTML), "utf8");
    const paginationIds = await pageEvaluate(pageClient, async () => {
      const database = await import(chrome.runtime.getURL("background/database.js"));
      const { PUBLIC_TRIAL_KEY_ID } = await import(chrome.runtime.getURL("shared/trial-key.js"));
      const ids = [];
      await database.withStrictReadWrite([database.KEY_STORE], async (transaction) => {
        const store = transaction.objectStore(database.KEY_STORE);
        const trial = await database.requestResult(store.get(PUBLIC_TRIAL_KEY_ID));
        for (let index = 0; index < 100; index += 1) {
          const bytes = new Uint8Array(16); bytes[15] = index;
          const keyId = btoa(String.fromCharCode(...bytes)).replaceAll("+", "-").replaceAll("/", "_").replaceAll("=", "");
          ids.push(keyId);
          await database.requestResult(store.add({ ...trial, keyId, displayName: `Pagination fixture ${index}`,
            keyKind: "regular", enabled: false, storedApiKey: null, secretVerifier: null }));
        }
      });
      document.querySelector("[data-refresh]").click();
      return ids;
    });
    await waitForCondition(pageClient, () => document.querySelectorAll("[data-key-rows] tr").length,
      (value) => value === 100, "first page filled by ordinary Keys");
    assert.equal(await pageEvaluate(pageClient, ({ id }) => document.querySelector(`[data-key-id="${id}"]`) !== null,
      { id: trialKeyId }), false);
    const paginatedTrial = await pageEvaluate(pageClient, trialSnapshot);
    assert.equal(paginatedTrial.warningVisible, true);
    trialSamples.push({ step: "trial_outside_first_100_rows", ...paginatedTrial });
    await pageEvaluate(pageClient, async ({ ids }) => {
      const database = await import(chrome.runtime.getURL("background/database.js"));
      await database.withStrictReadWrite([database.KEY_STORE], async (transaction) => {
        for (const id of ids) await database.requestResult(transaction.objectStore(database.KEY_STORE).delete(id));
      });
      document.querySelector("[data-refresh]").click();
    }, { ids: paginationIds });
    await waitForCondition(pageClient, () => document.querySelectorAll("[data-key-rows] tr").length,
      (value) => value === 1, "pagination fixture cleanup");
    ...
    const localizedTrial = await pageEvaluate(pageClient, trialSnapshot);
    assert.match(localizedTrial.warning, /公开.*撤销/su);
    assert.equal(localizedTrial.warningVisible, true);
    trialSamples.push({ step: "chinese_warning", ...localizedTrial });
    ...
    await waitForCondition(pageClient, ({ id }) => document.querySelector(`[data-key-id="${id}"] [data-key-state]`)?.dataset.keyState,
      (value) => value === "disabled", "public trial Key disabled by normal UI", { id: trialKeyId });
    ...
    const recoveredAdmin = await waitForCondition(
      ...
      (value) => value.connection === "connected" && value.rowCount === 1,
    ...
    const disabledTrial = await pageEvaluate(pageClient, trialSnapshot);
    assert.equal(disabledTrial.auth, "KEY_DISABLED");
    assert.equal(disabledTrial.warningVisible, false);
    trialSamples.push({ step: "disabled_after_worker_restart", ...disabledTrial });
    await pageEvaluate(pageClient, ({ id }) => {
      document.querySelector(`[data-key-id="${id}"] [data-key-edit]`).click();
    ...
    await waitForCondition(pageClient, ({ id }) => document.querySelector(`[data-key-id="${id}"] [data-key-state]`)?.dataset.keyState,
      (value) => value === "active", "public trial Key re-enabled by normal UI", { id: trialKeyId });
    const enabledTrial = await pageEvaluate(pageClient, trialSnapshot);
    assert.equal(enabledTrial.auth, "accepted");
    assert.equal(enabledTrial.warningVisible, true);
    trialSamples.push({ step: "re_enabled", ...enabledTrial });
    await pageEvaluate(pageClient, ({ id }) => {
      document.querySelector(`[data-key-id="${id}"] [data-key-edit]`).click();
    ...
    await waitForCondition(pageClient, () => document.querySelector("[data-connection]")?.dataset.state,
      (value) => value === "disconnected", "worker stopped after public Key revocation");
    await pageEvaluate(pageClient, () => document.querySelector("[data-refresh]").click());
    await waitForAdminReady(pageClient);
    workerClient = (await waitForExtensionWorker(debugPort)).client;
    ...
    if (process.argv.includes("--trial-key-only")) {
      console.log(JSON.stringify({ ok: true, suite: "public-trial-key", observations: trialSamples.length, samples: trialSamplesPath }));
      return;
    }
