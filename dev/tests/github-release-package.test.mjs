...
const manifest = JSON.parse(await readFile(path.join(workspaceRoot, "extension", "manifest.json"), "utf8"));
...
for (const relativeFile of [
  ...
  "background/virtual-input-service.js",
  "background/virtual-mouse-shortcuts.js",
  "shared/trial-key.js",
...
assert.match(extensionStart, new RegExp(`Version: \`${escapeRegExp(version)}\``, "u"));
assert.match(extensionStart, new RegExp(`${escapeRegExp(appName)}\\.zip`, "u"));
assert.match(extensionStart, /Chrome Web Store distribution uses a separate package and review process/u);
...
assert.equal((await readFile(path.join(appRoot, "windows-x86_64", "virtual-mouse-hook.dll"))).subarray(0, 2).toString("ascii"), "MZ");
assert.match(await readFile(path.join(appRoot, "licenses", "MinHook.txt"), "utf8"), /Tsuda Kageyu/u);
...
assert.match(appStart, /Linux provides browser routing and file workflows, not native input backends/u);
