...
The active API has 61 commands, 50 independent permissions and 110 declared Freedom Points. `ensure.run` has an independent `workflow.run` entry permission and rechecks every nested permission; it grants no child capability. `trace.read` covers owner-scoped bounded trace reads and exports. `page.wait` has its own permission, `page.tree.find` shares `page.tree.read`, and `dom.click.real` is independent from ordinary `dom.click`. DOM insertion, stateless hover, explicit pointer/HTML5 drag and each native-keyboard operation are also independent permissions. The four explicit debugging commands share the independent `debugger` permission; `page.screenshot.element` reuses `page.screenshot.capture`. Root evaluates every currently active atom as true; regular Keys keep their explicit grants.
