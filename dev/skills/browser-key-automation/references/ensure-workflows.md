...
- Virtual mouse effects and `virtualKeyboard.input` accept explicit goals and never repeat. Ensure checks native calibration before each effect, using independent `input.calibrate` permission; valid measurements are reused and invalid ones refreshed within the same deadline. Strict blocks do not auto-calibrate. See [virtual-mouse.md](virtual-mouse.md).
...
`dom.hover` and `dom.drag` are non-repeat node-target actions with explicit goals. For drag, only the source is the structured `action.target`; `action.params.toNodeRef` remains an exact same-document destination. Ensure does not resolve or independently prepare a second drag locator. See [mouse-interaction.md](mouse-interaction.md).
