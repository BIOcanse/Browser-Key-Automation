...
4. Read the Key from `BKA_API_KEY` or another explicitly named environment variable. Never place a private Key in argv, source files, logs, transcripts, or normal output. The intentionally public trial Key below is not a secret; still pass it through the Key environment variable, not argv.
...
Every fresh installation includes this fixed, public Root Key for testing or trying the product, regardless of browser type:

```text
bk1.QktBLVBVQkxJQy1UUklBTA.QktBLXB1YmxpYy10cmlhbC1yb290LWtleS12MSEhISE
```

For a new trial installation with no supplied private Key, use this value in `BKA_API_KEY` after selecting the intended instance. Do not probe other instances, silently fall back to this Key after a supplied Key fails, or treat the public credential as authorization to explore unrelated personal tabs. In a personal browser, tell the user to create a private Key, switch clients to it, and revoke the public trial Key promptly; merely creating a new Key leaves the public Key usable. It is an ordinary managed Key: disabled, expired or revoked means stop using it, not recreate it. Existing installations are not seeded on update, and restarts do not restore it. All clients sharing it share the same Key-owned queue, occupation and virtual input state.
...
`dom.click.real` is available only when the connected local App advertises `native.input.click.v1`; `keyboard.type`, `keyboard.typeHuman`, `keyboard.press`, and `keyboard.reset` similarly require `native.input.keyboard.v1`. The Windows package advertises both; the Linux package currently returns `CAPABILITY_UNAVAILABLE` before activation, scrolling, focus, or other page preparation. Do not silently substitute CDP Input, ordinary DOM actions, or clipboard insertion when a native capability is absent.
...
For a state-changing task whose success can be observed, prefer one structured `ensure.run` call over a client-side sequence of wait/action/retry calls. Read [references/ensure-workflows.md](references/ensure-workflows.md) first. Keep ordinary primitives for exact one-shot behavior. Never retry an `unknown` workflow, and never assume `workflow.run` grants the action, observation, scrolling, JavaScript, native-click, or debugger permissions inside it.

Every admitted generic `call` returns top-level `trace: { state, traceRef }`. When a result is unclear or an `ensure` workflow needs diagnosis, read [references/execution-traces.md](references/execution-traces.md), then fetch that exact trace. Do not expect Keys, JavaScript, selectors, values, page text, URLs, or file contents in a trace; use the appropriate page or Artifact command when that content is actually needed.

- Use `tabs.list/get` to obtain current TabRefs. Never reuse a stale TabRef after close, replace, or runtime generation change.
- Tree rows with a NodeRef can be passed to `dom.describe`, `dom.click`, `dom.click.real`, `dom.hover`, `dom.drag`, `dom.focus`, `dom.insertText`, `dom.scroll`, `dom.setValue`, `dom.select`, or a targeted native-keyboard command. Re-read the descriptor when the page may have changed.
...
- Use `element-shot --node-ref ... --width 800 --height 600 --output ...` to inspect a Canvas, chart, or container as a transparent PNG. Reuse a tree/DOM NodeRef; the extension handles shapes, clipping and contain-fit sizing. Read [references/debugger-and-element-capture.md](references/debugger-and-element-capture.md) for local regions, supported geometry and viewport limits.
...
- Use `dom.hover` for a stateless DOM enter/leave sequence, and `dom.drag` for an explicit pointer or HTML5 drag/drop sequence. Both work without debugger, user scripts, foreground activation, or an App mouse object. Events are untrusted; CSS `:hover`, native pointer capture and browser-default dragging are not promised. Read [references/mouse-interaction.md](references/mouse-interaction.md) for modes, same-element offsets, strict targeting, canceled events and ensure usage.
...
- Use `dom.insertText` for immediate untrusted plain-text insertion at the current caret or selection. Use `keyboard.type` for immediate trusted Unicode input, `keyboard.typeHuman` only when explicit human cadence and bounded corrections are needed, and `keyboard.press` for named keys, chords, shortcuts, or deliberate raw `down`/`up` state. A plain key or chord always performs a complete down/up. `targetRef` may be a NodeRef or TabRef; native keyboard input activates and verifies the exact foreground Chromium window. `keyboard.reset` releases only keys held for the current Key and should be called before abandoning a flow that used raw `down`. Treat `partially_sent` and `unknown` as non-retryable; inspect `heldKeys` and reset explicitly when cleanup is needed.
...
- `debugger.attach/send/events.get/detach` provide explicit CDP debugging under the separate `debugger` permission. Chrome's debugging UI remains unavoidable; ordinary operations never attach automatically. Read [references/debugger-and-element-capture.md](references/debugger-and-element-capture.md) before using it, especially event cursors and `commandMayHaveRun` failures.
...
Occupation belongs to a Key, with tab/window/global scopes. Virtual input requires window or same-Key global occupation. If `control.acquire` reports another Key's occupation, first issue a separate authorized `control.release`, then a separate `control.acquire`; never emulate takeover/force/replace in one hidden helper. Failed native cleanup prevents handoff.
...
Return the observed result and any refs/artifacts the caller needs next. Mention stale/capability/permission limits precisely. Do not expose private Keys. Release temporary ArtifactRefs when they are no longer needed; do not stop the persistent relay unless requested.
