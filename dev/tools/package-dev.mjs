...
async function buildExtensionPackage() {
  ...
  try {
    ...
    await copyFile(
      path.join(workspaceRoot, "dev", "packaging", "dev", "extension-START-HERE.md"),
...
async function buildLocalAppPackage({
  ...
  try {
    ...
    if (executableName.endsWith(".exe")) {
      await copyFile(path.join(path.dirname(sourceExecutable), "virtual-mouse-hook.dll"), path.join(stagingRoot, "virtual-mouse-hook.dll"));
      await mkdir(path.join(stagingRoot, "licenses"), { recursive: true });
      await copyFile(path.join(workspaceRoot, "app", "third_party", "minhook", "LICENSE.txt"), path.join(stagingRoot, "licenses", "MinHook.txt"));
    }
    await copyFile(
      path.join(workspaceRoot, "app", "client", "src", "main.mjs"),
      path.join(stagingRoot, "client", "browser-key-cli.mjs"),
    );
    await copyFile(
      path.join(workspaceRoot, "app", "client", "src", "native-websocket.mjs"),
    ...
    for (const name of ["generated-config.mjs", "artifact-files.mjs", "demo-files.mjs"]) {
      await copyFile(path.join(workspaceRoot, "app", "client", "src", name), path.join(stagingRoot, "client", name));
    }
    await copyFile(
      path.join(workspaceRoot, "dev", "protocol", "transport-profile.json"),
      path.join(stagingRoot, "protocol", "transport-profile.json"),
    );
    await cp(
      path.join(workspaceRoot, "dev", "skills", "browser-key-automation"),
    ...
    const template = await readFile(
      path.join(workspaceRoot, "dev", "packaging", "dev", "local-app-START-HERE.md"),
      "utf8",
    );
