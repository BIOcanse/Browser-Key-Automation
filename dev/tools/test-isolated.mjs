...
const queueProbe = forwardedArguments.includes("--native-boundary=html5-queue-diag");
const projectionProbe = forwardedArguments.includes("--native-boundary=html5-state-diag");
const settledProbe = forwardedArguments.includes("--native-boundary=html5-settle-diag");
const inputDiagnostic = settledProbe ? "settled" : projectionProbe ? "projection" : queueProbe ? "queue" : forwardedArguments.includes("--native-boundary=html5-diag") ? "observe" : "off";
const inputDiagnostics = inputDiagnostic !== "off";
...
try {
  for (const relative of ["app/client/src", "app/src", "app/third_party", "extension/manifest.json", "out/extension", "dev/registries", "dev/tests/lib", "app/build.zig"]) {
  ...
  for (const script of Object.values(suites)) {
    await cp(path.join(workspace, "dev", "tests", script), path.join(root, "dev", "tests", script));
  }
  const rootProbe = forwardedArguments.some((value) => ["--native-boundary=occlusion-root", "--native-boundary=occlusion-accessibility", "--native-boundary=occlusion-calibrated"].includes(value));
  if (inputDiagnostics || rootProbe) {
    await cp(path.join(workspace, "dev/tests/experiments/virtual-mouse"), path.join(root, "dev/tests/experiments/virtual-mouse"), { recursive: true });
  }
  if (rootProbe) {
  ...
  if (selected !== "extension") await run(process.env.BKA_ZIG_PATH ?? "zig", ["build", "--build-file", "app/build.zig", "--cache-dir", ".zig-cache", "--prefix", "zig-out", "-Doptimize=ReleaseSafe", ...(inputDiagnostics ? [`-Dvirtual-input-diagnostics=${inputDiagnostic}`] : [])], "build-relay");
  await new Promise((resolve) => reservation.close(resolve));
  const names = selected === "all" ? Object.keys(suites) : [selected];
  for (const name of names) await run(
    process.execPath,
    [path.join(root, "dev", "tests", suites[name]), ...(selected === "all" ? [] : forwardedArguments)],
    name,
  );
