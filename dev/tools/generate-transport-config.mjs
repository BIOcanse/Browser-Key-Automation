...
const freedomRegistry = JSON.parse(
  await readFile(path.join(workspaceRoot, "dev", "registries", "freedom.registry.json"), "utf8"),
);
const profile = JSON.parse(
  await readFile(path.join(workspaceRoot, "dev", "protocol", "transport-profile.json"), "utf8"),
);
const manifest = JSON.parse(
  await readFile(path.join(workspaceRoot, "extension", "manifest.json"), "utf8"),
);
const capabilityRegistry = JSON.parse(
  await readFile(path.join(workspaceRoot, "dev", "registries", "capabilities.registry.json"), "utf8"),
);
...
const virtualMouse = Object.fromEntries(["default_timeout_ms", "maximum_timeout_ms", "maximum_wait_ms", "maximum_objects", "maximum_actions", "cleanup_timeout_ms", "retirement_poll_ms"]
  .map((name) => [name, clientInteger(`command.virtualMouse.${name}`)]));
const localRoute = {
  maximumDurationMs: clientInteger("transport.local_route.maximum_duration_ms"),
  responseTimeoutMs: clientInteger("transport.local_route.response_timeout_ms"),
};
if (profile.localRouteCapability !== "route.local.v1") throw new Error("Local route capability must match its v1 contract");
if (profile.nativeRecordingCapability !== "native.recording.v1" || !Number.isSafeInteger(profile.maximumNativeRecordingBatchEvents) ||
    profile.maximumNativeRecordingBatchEvents < 1 || profile.maximumNativeRecordingBatchEvents * 1600 + 8192 > profile.maximumMessageBytes)
  throw new Error("Invalid bounded native recording transport profile");

const zigString = (value) => JSON.stringify(value);
const zig = `// Generated by dev/tools/generate-transport-config.mjs. Do not edit.
...
pub const virtual_mouse_capability = ${zigString(profile.virtualMouseCapability)};
pub const local_route_capability = ${zigString(profile.localRouteCapability)};
pub const native_recording_capability = ${zigString(profile.nativeRecordingCapability)};
pub const recording_maximum_batch_events: usize = ${profile.maximumNativeRecordingBatchEvents};
pub const recording_maximum_duration_ms: u32 = ${clientInteger("command.recording.maximum_duration_ms")};
pub const local_route_maximum_duration_ms: u32 = ${localRoute.maximumDurationMs};
${Object.entries(virtualMouse).map(([name, value]) => `pub const virtual_mouse_${name}: ${name.includes("timeout") ? "u32" : "usize"} = ${value};`).join("\n")}
...
const ts = `// Generated by dev/tools/generate-transport-config.mjs. Do not edit.
export const TRANSPORT_CONFIG = ${JSON.stringify(
  {
    ...
    virtualMouseCapability: profile.virtualMouseCapability,
    localRouteCapability: profile.localRouteCapability,
    nativeRecordingCapability: profile.nativeRecordingCapability,
    maximumNativeRecordingBatchEvents: profile.maximumNativeRecordingBatchEvents,
    localRoute,
    maximumPendingRoutes: profile.maximumPendingRoutes,
    virtualMouse,
...
await writeFile(
  path.join(workspaceRoot, "app", "src", "generated_config.zig"),
  zig,
  "utf8",
);
await writeFile(path.join(workspaceRoot, "app", "client", "src", "generated-config.mjs"),
...
await mkdir(path.join(workspaceRoot, "extension", "src", "generated"), { recursive: true });
await writeFile(
  path.join(workspaceRoot, "extension", "src", "generated", "transport-config.ts"),
  ts,
  "utf8",
);
