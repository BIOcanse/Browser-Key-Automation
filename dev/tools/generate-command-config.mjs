...
const commandRegistry = await readJson("dev/registries/commands.registry.json");
const capabilityRegistry = await readJson("dev/registries/capabilities.registry.json");
const errorRegistry = await readJson("dev/registries/errors.registry.json");
const freedomRegistry = await readJson("dev/registries/freedom.registry.json");
const manifest = await readJson("extension/manifest.json");
const transportProfile = await readJson("dev/protocol/transport-profile.json");
...
function validateParameterDefault(field, value) {
  ...
  const valid = field.valueType === "boolean" ? typeof value === "boolean" :
    ...
    field.valueType === "mouse_point_array" ? Array.isArray(value) && value.length === 0 :
    field.valueType === "signed_16_integer" ? integer && value >= -32768 && value <= 32767 :
...
if (sandboxCapability !== undefined) {
  ...
  for (const page of sandboxCapability.sandboxPages) await access(path.join(workspaceRoot, "extension", "static", page));
}
...
if (limits["command.dom.drag.default_steps"] > limits["command.dom.drag.maximum_steps"]) {
  throw new Error("DOM drag default_steps must not exceed maximum_steps");
}
...
const publicTrialKey = defaultPointValue("build.keys.public_trial_key");
if (typeof publicTrialKey !== "string" || !/^bk1\.[A-Za-z0-9_-]{22}\.[A-Za-z0-9_-]{43}$/u.test(publicTrialKey) ||
    publicTrialKey.split(".").slice(1).some((part, index) => {
      const bytes = Buffer.from(part, "base64url");
      return bytes.length !== (index === 0 ? 16 : 32) || bytes.toString("base64url") !== part;
    })) throw new Error("Public trial Key must be a canonical bk1 credential");

const generated = `// Generated by dev/tools/generate-command-config.mjs. Do not edit.
export const PUBLIC_TRIAL_KEY = ${JSON.stringify(publicTrialKey)} as const;
...
await writeFile(path.join(workspaceRoot, "extension", "src", "generated", "command-config.ts"), generated, "utf8");
await copyFile(
  path.join(workspaceRoot, "dev", "registries", "commands.registry.json"),
  path.join(workspaceRoot, "dev", "skills", "browser-key-automation", "references", "commands.registry.json"),
);
await copyFile(
  path.join(workspaceRoot, "dev", "registries", "freedom.registry.json"),
  path.join(workspaceRoot, "dev", "skills", "browser-key-automation", "references", "freedom.registry.json"),
);
