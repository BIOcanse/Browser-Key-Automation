...
const sourceManifest = JSON.parse(
  await readFile(path.join(workspaceRoot, "extension", "manifest.json"), "utf8"),
);
...
try {
  ...
  await copyFile(path.join(extracted.windows, "virtual-mouse-hook.dll"), path.join(appRoot, "windows-x86_64", "virtual-mouse-hook.dll"));
  await mkdir(path.join(appRoot, "licenses"), { recursive: true });
  await copyFile(path.join(extracted.windows, "licenses", "MinHook.txt"), path.join(appRoot, "licenses", "MinHook.txt"));
...
async function compareSharedAppFiles(windowsRoot, linuxRoot) {
  const excluded = new Set(["START-HERE.md", "SHA256SUMS.txt", "browser-key-relay.exe", "browser-key-relay", "virtual-mouse-hook.dll", path.join("licenses", "MinHook.txt")]);
...
async function renderTemplate(name, values) {
  let text = await readFile(path.join(workspaceRoot, "dev", "packaging", "release", name), "utf8");
...
function releaseNotes(releaseVersion, assets) {
  const lines = [
    ...
    "",
    "Work in your existing signed-in Chromium tabs with selective page trees, practical file workflows and optional debugging. Routine extension commands do not attach a debugger or add per-command confirmation.",
    "",
    "## New in this release",
    "",
    "- Offline App detection no longer repeatedly adds WebSocket failures to the extension error list. Automatic connection and reconnection remain enabled. Update the extension and companion App together.",
    ...
    "- One authorization check at command submission: accepted actions continue in the execution queue when their Key later expires, is disabled or revoked, or has its permissions changed. New submissions use the current Key permissions.",
    "- Browser tools for file uploads, downloads, dialogs, network inspection and interception, console and performance diagnostics, document/element screenshots, and browser-data search across tabs, bookmarks and history.",
    "- Explicit window, viewport and zoom checks and corrections compose with `ensure.run` before recorded actions. Updated management translations, all ten README variants and the bundled Agent guide describe the workflows.",
    "",
    "## Continuing capabilities",
    "",
    "Cached, selectively expanded page trees; cross-tab actions; scoped permission groups; exact and human-paced keyboard input; one-command MHTML saving; resource transfer; serverless HTML demos; and explicit CDP commands/events remain available. `input.calibrate` remains the explicit page-coordinate calibration command; `keyboard.reset` releases the calling Key's held keyboard state.",
    "",
    "Fresh installations include a public Root trial Key. Create a private Key, switch clients, then revoke the trial Key; creating a new Key alone does not disable it, and updates do not recreate it. The bundled Agent skill contains the public value and setup instructions.",
    "",
    "## Platform boundaries",
    "",
    "- Windows x64 includes real recording, native input and the virtual-input hook DLL. Recording is observed by the external App. Linux x64 provides browser routing and file workflows, not native recording or mouse/keyboard input.",
    "- External native recording captures keyboard keys, scan codes, layout and repeat, but not IME committed text. Incomplete gestures and legacy screen-coordinate recording drafts produce compile diagnostics; old drafts need a new recording. Existing saved actions keep their original commands.",
    "- Minimized windows are unsupported for native input. Page-coordinate input still needs a measurable calibration; `virtualMouse.moveWindow` uses window coordinates directly. Full native HTML5/OLE drag/drop and some multiwindow page-calibration cases remain unresolved.",
    "- Synthetic DOM input, virtual window messages and shape-masked viewport captures are not universal substitutes for OS input or hidden-page rendering. An accepted input does not prove the website completed its business action.",
    "- Explicit `debugger.attach` retains Chrome\'s debugging UI. Browser-owned site permissions and User Scripts enablement still apply.",
    "",
    "## Downloads",
    "",
    "This release has exactly two downloads:",
    "",
    ...
    "",
    "These GitHub packages are for manual installation. Chrome Web Store distribution uses a separate package and review process.",
    "",
    "## SHA-256",
    "",
    "```text",
