...
function releaseNotes(releaseVersion, assets) {
  const lines = [
    ...
    "## New in this release",
    "",
    "- Independent Windows virtual mouse and keyboard commands: persistent Key-owned state across tabs, cursor/button/wheel primitives, click/drag/scroll shortcuts, and element-to-element dragDrop. Whole-window occupation is required. Physical cursor position is unchanged.",
    "- Explicit `input.calibrate` for virtual input. `ensure.run` validates calibration before an effect, refreshes it when needed, and reuses a valid measurement. `keyboard.reset` releases the calling Key\'s held keyboard state.",
    "- Ordinary `dom.hover` and `dom.drag` remain separate synthetic DOM operations, without silently switching to a native or debugger backend.",
    "- Fresh installations include a public Root trial Key and localized management warnings. For a personal browser: create a private Key, switch clients, then revoke the trial Key. Creating a new Key alone does not disable it; updates do not recreate it. The public value and usage instructions are in the bundled Agent skill.",
    "- Source layout is now `app/`, `extension/` and `dev/`. All ten README language variants, installation guidance and privacy disclosures have been rewritten around practical workflows.",
    "",
    "## Continuing capabilities",
    "",
    "Cached, selectively expanded page trees; cross-tab actions; scoped permission groups; exact and human-paced keyboard input; one-command MHTML saving; viewport and transparent element images; resource transfer; serverless HTML demos; and explicit CDP commands/events remain available.",
    "",
    "## Platform boundaries",
    "",
    "- Windows x64 includes native input and the required hook DLL. Linux x64 provides browser routing and file workflows, not native mouse/keyboard input.",
    "- Minimized windows are unsupported. First virtual-input calibration needs a measurable page; reusing calibration while covered is conditional. Multiwindow first calibration and full native HTML5/OLE drag/drop still have unresolved cases.",
