...
首次安装自带所有安装共用的公开 Root Key，仅供测试或试用。若安装在个人浏览器中，请尽快创建私人 Key、切换调用方并撤销 `Public trial Key`。仅创建新 Key 不会使旧 Key 失效；禁用/撤销后，更新和重启不会恢复它。旧安装升级不插入公开 Key。
...
5. 首次安装会自动打开本地介绍页，按页面指引完成设置。以后可从 Key 管理页的“安装与使用说明”重新打开；更新/重载不会重复弹出。打开扩展详情并核对：
   ...
   - 版本：与本目录 `manifest.json` 的 `version` 一致
...
8. 仅测试/试用时可直接在 `Public trial Key` 行复制默认 Key。要创建私人 Key，点击右上角 **创建 Key**，选择需要的权限或 Root 类型、按需填写有效期，然后创建；调用方切换到新 Key 后，编辑并撤销公开试用 Key。
9. 完整 Key 保存在这个扩展的本地数据库中；以后可在对应行点击“显示”或“复制”，隐藏、刷新或关闭页面不会删除它。旧开发版创建且仅有 verifier 的记录会显示“补存现有 Key”，可输入手中原始 Key 完成本地保存。
