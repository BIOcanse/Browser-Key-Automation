...
这是与浏览器扩展分开交付的本地 App，负责 loopback WebSocket 路由、实例编号和平台原生输入。Key 鉴权与浏览器权限由扩展管理；App 保存运行中的路由与 Key 绑定输入状态，不保存另一套 Key 数据库。

全新安装扩展会带有公开 Root 试用 Key。个人浏览器请创建私人 Key、切换客户端，再撤销试用 Key；只创建新 Key 不会使旧 Key 失效。公开值及试用说明见包内 skill。

Windows 提供真实与虚拟键鼠，需保留程序旁的 `virtual-mouse-hook.dll`。Linux 当前提供浏览器路由和文件流程，不提供原生键鼠后端。虚拟输入要求占据整个目标窗口，并在普通操作前使用 `input.calibrate`；`ensure.run` 会按需检查和更新校准。不支持最小化窗口，多窗口首次校准及完整原生 HTML5/OLE 拖放仍有未解决场景。
