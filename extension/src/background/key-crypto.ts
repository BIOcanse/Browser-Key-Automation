...
export async function generateKeyMaterial(): Promise<GeneratedKeyMaterial> {
  return createKeyMaterial(`bk1.${encodeBase64Url(randomBytes(KEY_ID_BYTE_LENGTH))}.${encodeBase64Url(randomBytes(SECRET_BYTE_LENGTH))}`);
}

export async function createKeyMaterial(apiKey: string): Promise<GeneratedKeyMaterial> {
  const parsed = parseApiKey(apiKey);
  if (parsed === null) throw new Error("Invalid API Key material");
  const saltBytes = randomBytes(SALT_BYTE_LENGTH);
  const digest = await verifierDigest(parsed.keyIdBytes, saltBytes, parsed.secretBytes);
  return {
    keyId: parsed.keyId,
    apiKey,
    verifier: {
