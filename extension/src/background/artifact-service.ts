...
export async function createArtifact(ownerKeyId: string, mediaType: string, bytes: Blob, diagnostic?: StorageDiagnostic): Promise<ArtifactMetadata> {
  const initialSettings = await storageStage(diagnostic, "settings.read", getRuntimeSettings);
  if (
  ...
  const buffer = await storageStage(diagnostic, "blob.read", () => bytes.arrayBuffer(), { byteLength: bytes.size });
  const sha256 = await storageStage(diagnostic, "digest", () => digestHex(buffer), { byteLength: buffer.byteLength });
  ...
  while (attempt < GENERATION_ATTEMPTS) {
    ...
    const result = await withStrictReadWrite(
      ...
      async (transaction): Promise<ArtifactMetadata | null> => {
        ...
        await storageStage(diagnostic, "metadata.add", () => requestResult(store.add(record)));
        ...
        while (chunkIndex < chunkCount) {
          ...
          await storageStage(diagnostic, "chunk.add", () => requestResult(chunkStore.add(chunk)), { chunkIndex, chunkCount, byteLength: chunk.bytes.byteLength });
          chunkIndex += 1;
        }
