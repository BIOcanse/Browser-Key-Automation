...
export interface EnsureDependencies {
  ...
  readonly executeAction: (action: EnsureAction, nodeRef: string | null, timeoutMs: number) => Promise<unknown>;
  readonly prepareAction?: (action: EnsureAction, nodeRef: string | null, timeoutMs: number) => Promise<void>;
  ...
  try {
    if (request.mode === "strict") {
      ...
      try {
        ...
        result = ended || budget < 1 ? expired : await within(dependencies.executeAction(request.action, nodeRef, budget));
      }
      catch (error) {
    ...
    actionLoop: while (!ended) {
      ...
      currentStage = "effect";
      if (dependencies.prepareAction !== undefined) {
        currentStage = "prepare";
        if (await within(dependencies.prepareAction(request.action, admittedNodeRef, Math.max(1, Math.floor(started + request.timeoutMs - dependencies.now())))) === expired) return finish("failed", "prepare");
        if (await within(dependencies.authorize(request.action.requiredPermission)) === expired) return finish("failed", "prepare");
      }
      if (ended || dependencies.now() >= started + request.timeoutMs) return finish("failed", "prepare");
      currentStage = "effect";
      effectSent = true;
      effectAttempts += 1;
      ...
      try {
        const budget = Math.floor(started + request.timeoutMs - dependencies.now());
        executed = ended || budget < 1 ? expired : await within(dependencies.executeAction(request.action, admittedNodeRef, budget));
      }
      catch (error) {
