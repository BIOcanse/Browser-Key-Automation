...
import { observeStorage, storageStage, type StorageDiagnostic } from "./database.js";
...
export async function capturePageArchive(
  ownerKeyId: string,
  tabRef: string,
  trace?: { readonly id: string; readonly record: (phase: string, status: string) => void },
): Promise<{ readonly tabRef: string; readonly artifact: ArtifactMetadata }> {
  const started = performance.now();
  const id = trace?.id ?? crypto.randomUUID();
  let firstFailure = false, samples = 0;
  const safeText = (value: unknown): string | null => typeof value === "string" ? boundedText(value
    .replace(/bk1\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+/gu, "[Key redacted]")
    .replace(/https?:\/\/[^\s)]+/gu, "[URL redacted]"), limit("command.page.save.maximum_diagnostic_text_bytes")).value : null;
  const diagnostic: StorageDiagnostic = (phase, status, error, detail) => {
    const first = status === "failed" && !firstFailure;
    if (status === "failed") firstFailure = true;
    if (++samples > limit("command.trace.maximum_events") && !first) return;
    trace?.record(phase, status);
    console.info("BKA page-save diagnostic", JSON.stringify({ id, sequence: samples, phase, status, firstFailure: first,
      elapsedMs: Math.round((performance.now() - started) * 1000) / 1000,
      buildId: TRANSPORT_CONFIG.buildId, browser: navigator.userAgent, ...detail,
      ...(error === undefined ? {} : { error: { name: safeText(error instanceof Error ? error.name : typeof error),
        message: safeText(error instanceof Error ? error.message : String(error)), stack: safeText(error instanceof Error ? error.stack : null) } }),
    }));
  };
  const target = await storageStage(diagnostic, "target.resolve", () => resolveTabTarget(tabRef));
  assertResolvedTabTarget(target);
  let blob: Blob | undefined;
  try {
    blob = await storageStage(diagnostic, "capture.mhtml", () => chrome.pageCapture.saveAsMHTML({ tabId: target.tabId }));
  } catch {
    assertResolvedTabTarget(target);
    throw new CapabilityUnavailableError(
  ...
  observeStorage(diagnostic, "capture.blob", "succeeded", undefined, { byteLength: blob.size, mediaType: blob.type });
  const artifact = await createArtifact(ownerKeyId, "multipart/related", blob, diagnostic);
  observeStorage(diagnostic, "archive.complete", "succeeded", undefined, { byteLength: artifact.byteLength });
