...
import { createKeyMaterial, digestCanonicalText, generateKeyMaterial, parseApiKey, verifyParsedApiKey, type ParsedApiKey } from "./key-crypto.js";
import { PUBLIC_TRIAL_KEY, PUBLIC_TRIAL_KEY_ID, PUBLIC_TRIAL_KEY_NAME } from "../shared/trial-key.js";
import {
  createKeyRecord,
  hasPermission,
  storedApiKey,
...
export async function initializePublicTrialKey(reason: ChromeInstalledDetails["reason"]): Promise<boolean> {
  ...
  return withStrictReadWrite([KEY_STORE], async (transaction) => {
    ...
    await requestResult(store.add(createKeyRecord({
      ...
      secretVerifier: material.verifier, storedApiKey: material.apiKey,
    })));
    return true;
  });
}
...
export async function listKeysForAdmin(params: ListKeysParams): Promise<AdminListKeysResult> {
  const page = await listKeys(params);
  const trial = await getKeyById(PUBLIC_TRIAL_KEY_ID);
  return { ...page, publicTrialKeyActive: trial !== undefined && trial.status === "active" && trial.enabled &&
    trial.secretVerifier !== null && (trial.expiresAt === null || trial.expiresAt > Date.now()) };
}
