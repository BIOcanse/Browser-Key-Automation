...
export interface KeyboardTargetDispatchContext extends KeyboardDispatchContext {
  ...
  readonly admittedWindowId: number;
}
...
async function activateExactTab(tabRef: string, windowId: number): Promise<void> {
  const target = await resolveTabTarget(tabRef);
  assertResolvedTabTarget(target);
  if ((await chrome.tabs.get(target.tabId)).windowId !== windowId) throw failure("target_changed");
  try {
    const activated = await chrome.tabs.update(target.tabId, { active: true });
    if (activated.windowId !== windowId) throw failure("target_changed");
...
async function executeTargeted(
  ...
  await activateExactTab(tabRef, context.admittedWindowId);
...
export async function resetKeyboard(context: KeyboardDispatchContext): Promise<{
  ...
  const result = await resetKeyInput(context, "keyboard");
  return { status: "input_sent", completedActions: result.completedActions, heldKeys: result.keyboard.heldKeys };
}
