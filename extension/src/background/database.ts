...
export type StorageDiagnostic = (phase: string, status: "started" | "succeeded" | "failed", error?: unknown, detail?: Readonly<Record<string, string | number | null>>) => void;
export function observeStorage(observer: StorageDiagnostic | undefined, phase: string, status: "started" | "succeeded" | "failed", error?: unknown, detail?: Readonly<Record<string, string | number | null>>): void {
  try { observer?.(phase, status, error, detail); } catch { /* Diagnostics never alter the storage operation. */ }
}
export async function storageStage<T>(observer: StorageDiagnostic | undefined, phase: string, action: () => Promise<T>, detail?: Readonly<Record<string, string | number | null>>): Promise<T> {
  observeStorage(observer, phase, "started", undefined, detail);
  try { const result = await action(); observeStorage(observer, phase, "succeeded", undefined, detail); return result; }
  catch (error) { observeStorage(observer, phase, "failed", error, detail); throw error; }
}
...
export async function withStrictReadWrite<T>(
  ...
  try {
    const database = await storageStage(diagnostic, "database.open", () => within(getDatabase()));
    observeStorage(diagnostic, "transaction.begin", "started");
    try { transaction = database.transaction([...storeNames], "readwrite", { durability: "strict" }); }
    catch (error) { observeStorage(diagnostic, "transaction.begin", "failed", error); throw error; }
    observeStorage(diagnostic, "transaction.begin", "succeeded");
    completion = transactionCompletion(transaction);
    void completion.catch((error: unknown) => observeStorage(diagnostic, "transaction.abort", "failed", error));
    const result = await storageStage(diagnostic, "transaction.requests", () => within(operation(transaction!)));
    await storageStage(diagnostic, "transaction.commit", () => within(completion!));
    return result;
  } catch (error) {
    try {
      transaction?.abort();
