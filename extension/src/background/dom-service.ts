...
function markKeyboardWindowDocument(marker: string) {
  const page = globalThis as unknown as { readonly document: { title: string }; readonly innerWidth: number; readonly innerHeight: number };
  const originalTitle = page.document.title;
  page.document.title = marker;
  return { originalTitle, viewport: { width: page.innerWidth, height: page.innerHeight } };
}
...
export async function markKeyboardWindow(tabRef: string, marker: string): Promise<KeyboardWindowMarker & { readonly viewport: { readonly width: number; readonly height: number } }> {
  ...
  return { tabId: tabTarget.tabId, documentId: entry.documentId, ...entry.result };
}

export async function restoreKeyboardWindow(marker: KeyboardWindowMarker, expectedTitle: string, timeoutMs: number): Promise<void> {
  let timer: ReturnType<typeof setTimeout> | undefined;
  try {
    const restoring = chrome.scripting.executeScript({
      target: { tabId: marker.tabId, documentIds: [marker.documentId] },
      world: "ISOLATED",
      func: restoreRealClickDocumentTitle,
      args: [expectedTitle, marker.originalTitle],
    });
    // Native modal loops can defer scripting even after input delivery has
    // finished. Cleanup must not hold the Key's serial queue beyond its budget.
    // A late injection remains safe: exact document + expected title are checked.
    const expired = Symbol("expired");
    const outcome = await Promise.race([restoring, new Promise<typeof expired>((resolve) => {
      timer = setTimeout(() => resolve(expired), Math.max(0, timeoutMs));
    })]);
    if (outcome === expired) console.warn("BKA native title restore timed out; the original input result is unchanged");
  } catch {
    // A navigated/closed document cannot retain the temporary title marker.
