...
Работайте в Chromium-вкладках с уже выполненным входом: читайте, действуйте и сохраняйте результат. Обычные команды используют разрешения расширения: без отдельного браузера, автоматического подключения отладчика и собственного подтверждения каждой команды. Проверки самого браузера остаются.
...
BKA ориентирован на личный браузер, выборочное дерево и связанные операции с файлами. Это не универсальная замена тестовым фреймворкам или DevTools.
